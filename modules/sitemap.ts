...
export default defineNuxtModule({
	...
	async setup(options, nuxt) {
		async function generateSitemap(routes: NuxtPage[]): Promise<string> {
			...
			return streamToPromise(Readable.from(sitemapRoutes).pipe(stream)).then((data) => data.toString());
		}
		...
		const filePath = resolver.resolve(nuxt.options.srcDir, 'node_modules/.cache/.sitemap/sitemap.xml');
