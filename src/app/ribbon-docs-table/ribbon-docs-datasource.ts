...
const RIBBON_DATA: RibbonDocs[] = ribbondocs.map((docItem: IJSDocItem) => ({
  name: docItem.name,
  aliases:
    docItem.description && docItem.description.split('**Aliases**').length > 1
      ? docItem.description
          .split('**Aliases**')[1]
          .slice(3)
          .replace(/`/g, '')
...
export class RibbonDocsDatasource extends DataSource<RibbonDocs> {
  ...
  public connect(): Observable<RibbonDocs[]> {
    const dataMutations = [observableOf(this.data), this.paginator.page, this.sort.sortChange, this.filterChange];
